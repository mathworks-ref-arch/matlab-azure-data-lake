classdef testApiDoc < matlab.unittest.TestCase
    ...
    methods (Test)
        function testgenApiDoc(testCase)
            ...
            apiDocFull = [documentationDir,filesep,'AzureDataLakeApi.md'];
            ...
            % verify that the generation worked also, utilities directory
            ...
            if contains(diffOutput,'<div>The files are identical.</div>')
                write(testCase.logObj,'debug','MATLAB Interface for Azure Data Lake API documentation is up to date');
                diffResult = true;
            else
                write(testCase.logObj,'warning','MATLAB Interface for Azure Data Lake API documentation is NOT up to date');
                % debug output
